...
app.controller('personsCtrl', function ($scope) {
  ...
  $scope.remove = function(person) {
    ...
    addTexttoTextArea('Person deleted: ', person);
  }
  
  $scope.addPerson = function(person) { 
    if (person.hasOwnProperty('name') && person.hasOwnProperty('age')) {
      person.name = toTitleCase(person.name);
      if (person.hasOwnProperty('job')) {
        person.job = toTitleCase(person.job);
      }
      $scope.persons.push(person);
      addTexttoTextArea('New person added: ', person);
    }
    $scope.person = null;
  }
  
  $scope.isEmployee = function(person) {
    return person.employee ? '☑' : '☐';
  }
  
  var addTexttoTextArea = function (text, person) {
    var myDump = document.getElementById("mytextarea").value;
    var now = new Date();
    myDump = text + person.name + ' - ' + person.job + ' - at ' + now.toLocaleString('hu-HU') + '\n' + myDump;
    document.getElementById("mytextarea").value = myDump;
  }
});
